...
export function MobileBottomNav() {
  ...
  function navigate(destination: PrimaryNavigationDestination) {
    ...
    if (
      ...
      return;
    startNavigation();
    if (destination === "agent") return void router.push("/fashion-agent");
          ...
          ({ destination, labelKey, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`relative flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[11px] font-black leading-none transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/80 ${active ? "text-orange-400" : "text-gray-400 hover:bg-white/[0.04] hover:text-gray-100"}`}
              >
                {active && (
                  <span
                    className="absolute top-1.5 h-0.5 w-6 rounded-full bg-orange-400"
                    aria-hidden="true"
                  />
                )}
