...
function MySizesManager({
  ...
  const saveProfileNote = async (profileId: string) => {
    ...
    try {
      ...
      setEditError(error instanceof Error ? error.message : t("mysize.noteUpdateError"));
    } finally {
  ...
  const confirmDeleteProfile = async () => {
    ...
    try {
      ...
      setDeleteError(error instanceof Error ? error.message : t("mysize.deleteError"));
    } finally {
    ...
    <>
      ...
      <div className="mb-5 flex items-center justify-between gap-3">
        ...
        >
          ...
          {isAdding ? t("common.close") : t("common.add")}
        </button>
      </div>
      ...
      {isAdding && (
        <div className="my-size-panel-reveal mb-5 border-t border-white/[0.08] pt-4">
          <div>
            <label className="mb-2 block text-[11px] font-black uppercase tracking-wide text-gray-500">{t("mysize.selectItem")}</label>
              ...
              <div>
                    ...
                    {selectedProduct ? (
                      <>
                        ...
                        <p className="mt-0.5 truncate text-[11px] font-bold text-gray-500">
                          {selectedProduct.brand} · {selectedProduct.closetSelectedSizeLabel || selectedProduct.closetSelectedSizeSnapshot?.row?.[0] || t("mysize.sizeFallback")}
                        </p>
                      </>
                    ) : (
                      <>
                        <p className="text-xs font-black text-white">{t("mysize.chooseFromCloset")}</p>
                      </>
                    )}
                  </div>
                  {selectedProduct && (
                    <span className="inline-flex shrink-0 items-center gap-1 text-[11px] font-bold text-orange-200">
                      <Check className="h-3.5 w-3.5" aria-hidden="true" /> {t("mysize.selected")}
                    </span>
                  )}
                ...
                {isProductPickerOpen && (
                  <div id="my-size-product-list" className="mt-3 border-t border-white/[0.08] pt-3">
                    <label htmlFor="my-size-product-search" className="sr-only">{t("mysize.search")}</label>
                    <input
                      id="my-size-product-search"
                      value={productSearchQuery}
                      onChange={(event) => setProductSearchQuery(event.target.value)}
                      placeholder={t("mysize.search")}
                    ...
                    <div className="grid max-h-[210px] gap-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      {filteredClosetCandidates.length > 0 ? (
                        filteredClosetCandidates.map((product) => {
                          const sizeLabel = product.closetSelectedSizeLabel || product.closetSelectedSizeSnapshot?.row?.[0] || t("mysize.sizeFallback");
          ...
          <div className="mt-4">
            <label htmlFor="my-size-fit-note" className="mb-2 block text-[11px] font-black uppercase tracking-wide text-gray-500">
              {t("mysize.note")} <span className="normal-case font-semibold text-gray-600">{t("mysize.optional")}</span>
            </label>
          <textarea
            id="my-size-fit-note"
            value={fitNote}
            onChange={(event) => setFitNote(event.target.value)}
            placeholder={t("mysize.notePlaceholder")}
            rows={2}
          ...
          {!selectedProduct && closetCandidates.length > 0 && (
            <p id="my-size-save-hint" className="mt-3 text-xs font-semibold text-gray-500">{t("mysize.saveHint")}</p>
          )}
          ...
          >
            {isSaving ? t("mysize.saving") : t("mysize.save")}
          </button>
        </div>
      )}
                        ...
                        >
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            >
                              <span className="flex min-w-0 items-center gap-3">
                                <span className="min-w-0 flex-1">
                                  <span className="block truncate text-sm font-black text-white">{getMySizePrimaryLabel(profile, t("mysize.sizeFallback"))}</span>
                                  <span className="mt-0.5 block truncate text-xs font-semibold text-gray-500">
                                    {profile.brand ? `${profile.brand} · ` : ""}{profile.title || t("mysize.untitled")}
                                  </span>
                                </span>
                            ...
                            <button
                              ...
                              aria-label={t("mysize.editNoteAria")}
                            ...
                            <button
                              ...
                              aria-label={t("mysize.deleteAria")}
      ...
      {editingProfileId && noteEditorPosition && createPortal(
        <>
          <button
            type="button"
            aria-label={t("mysize.closeNoteEditorAria")}
            onClick={() => {
          ...
          >
            <h3 id="my-size-note-editor-title" className="text-sm font-black text-white">{t("mysize.editNoteTitle")}</h3>
            <textarea
              ref={noteEditorRef}
              value={editingFitNote}
              onChange={(event) => setEditingFitNote(event.target.value)}
              placeholder={t("mysize.editNotePlaceholder")}
              rows={3}
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              >
                {isUpdatingProfile ? t("mysize.saving") : t("common.save")}
              </button>
            </div>
          </div>
        </>,
      ...
      {deleteCandidate && createPortal(
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-red-500/20 bg-[#151518] p-6 text-center shadow-[0_24px_64px_rgba(0,0,0,0.68)]">
            ...
            <h3 id="my-size-delete-title" className="mt-4 text-lg font-black text-white">{t("mysize.deleteConfirmTitle")}</h3>
            <p className="mt-2 text-sm font-semibold leading-relaxed text-gray-400">
              {t("mysize.deleteConfirmDescription", {
                size: getMySizePrimaryLabel(deleteCandidate, t("mysize.sizeFallback")),
                title: deleteCandidate.title || t("comparison.savedProduct"),
              })}
            </p>
            <p className="mt-2 text-xs font-semibold text-red-300/80">{t("mysize.deleteConfirmWarning")}</p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-2">
              ...
              >
                {isDeletingProfile ? t("mysize.deleting") : t("common.delete")}
              </button>
            </div>
          </div>
        </div>,
      ...
      <section aria-labelledby="account-settings-title">
        ...
        <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#111114]">
          ...
          >
            ...
            <span className="min-w-0 flex-1"><span className="block text-sm font-black text-gray-200">{t("mypage.changeUsername")}</span><span className="mt-0.5 block truncate text-xs font-semibold text-gray-500">{t("mypage.current", { value: username })}</span></span>
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-black text-gray-200">{t("mypage.language")}</span>
              <span className="mt-0.5 block text-xs font-semibold text-gray-500">{t("mypage.languageSwitchHint", { current: locale === "ko" ? "한국어" : "English", next: alternateLocaleLabel })}</span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-gray-600" />
          </button>
          <div className="mx-4 border-t border-white/[0.07]" />
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-black text-gray-200">{t("mypage.logout")}</span>
              <span className="mt-0.5 block text-xs font-semibold text-gray-500">{t("mypage.logoutHint")}</span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-gray-600" />
          </button>
        </div>
        <div className="mt-5 overflow-hidden rounded-xl border border-red-500/20 bg-red-500/[0.035]">
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-black text-red-200">{isDeletingAccount ? t("mypage.deleting") : t("mypage.deleteAccount")}</span>
              <span className="mt-0.5 block text-xs font-semibold text-red-300/65">{t("mypage.deleteAccountHint")}</span>
            </span>
    ...
    {isLogoutConfirmOpen && (
      <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
        <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#151518] p-6 text-center shadow-[0_24px_64px_rgba(0,0,0,0.65)]">
          ...
          <h2 className="mt-4 text-lg font-black text-white">{t("mypage.logoutConfirm")}</h2>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-gray-400">
            {t("mypage.logoutConfirmDescription")}
          </p>
          <div className="mt-6 grid grid-cols-2 gap-2">
            <button
              type="button"
    ...
    {isDeleteConfirmOpen && (
      <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 px-4 backdrop-blur-sm">
        <div className="w-full max-w-sm rounded-2xl border border-red-500/20 bg-[#151518] p-6 text-center shadow-[0_24px_64px_rgba(0,0,0,0.68)]">
          ...
          <h2 className="mt-4 text-lg font-black text-white">{t("mypage.deleteConfirm")}</h2>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-gray-400">
            {t("mypage.deleteConfirmDescription")}
          </p>
