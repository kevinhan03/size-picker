...
const ClosetCollectionContent = dynamic(
    ...
    ),
  {
    ssr: false,
    loading: () => (
      <div aria-busy="true" className="profile-posts-placeholder" />
    ),
  }
);
...
export function ProfileView({
  ...
  const [socialProfile, setSocialProfile] =
    useState<SocialProfileSummary | null>(null);
  ...
  return (
    <main className="profile-page">
      <div className="profile-shell">
        ...
        {signature ? (
          <section
            className="profile-taste profile-taste-signature"
            aria-labelledby="profile-taste-title"
          >
            <div className="profile-section-heading">
              <h2 id="profile-taste-title">
                {c.signatureTitle.replace("{username}", profile.username)}
              </h2>
            </div>
            ...
            {signature.axes.length ? (
              <div className="profile-signature-group">
                <h3>{c.signaturePoints}</h3>
                <div className="profile-signature-points">
            ...
            {signature.details.length ? (
              <div className="profile-signature-group">
                <h3>{c.signatureDetails}</h3>
                <p className="profile-signature-details">
                  {signature.details.slice(0, 2).join(" · ")}
                </p>
              </div>
            ) : null}
            {isOwner ? (
          ...
          <section
            className="profile-taste profile-taste-empty"
            aria-labelledby="profile-taste-title"
          >
            <h2 id="profile-taste-title">
              {c.signatureTitle.replace("{username}", profile.username)}
        ...
        >
          {isOwner ? (
            contentTab === "closet" && (
              <ClosetCollectionContent
                initialProducts={initialCloset}
                active={contentTab === "closet"}
              />
            )
          ) : profile.closetIsPublic ? (
            profile.closetProducts?.length ? (
              <div className="profile-public-closet-grid">
                {profile.closetProducts.map((product) => (
                  <Link key={product.id} href={getProductPageUrl(product)}>
                    <img
                      src={
                        product.thumbnailImage ||
                        product.image ||
              ...
              <p className="profile-posts-placeholder">{c.publicClosetEmpty}</p>
            )
          ) : (
