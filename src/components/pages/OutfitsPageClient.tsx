...
const tabs: Array<{ value: HubScope }> = [{ value: "open" }, { value: "mine" }, { value: "proposed" }];
...
export function OutfitsPageClient({ initialScope = "open", initialData = null }: { initialScope?: HubScope; initialData?: { requests: OutfitRequestSummary[]; total: number; nextCursor: string | null } | null }) {
  ...
  if (isAuthLoading || (!authUser && !error)) {
    return <main className="flex min-h-screen items-center bg-black px-4 pt-[var(--app-main-pt)]"><PageState kind="loading" title={t("outfits.loading")} description={t("outfits.loadingDescription")} /></main>;
  }

  const mineEmptyState = mineStatus === "all"
    ? { title: t("outfits.empty.mine.all.title"), description: t("outfits.empty.mine.all.description"), ctaLabel: t("outfits.empty.mine.all.cta") }
    : mineStatus === "open"
      ? { title: t("outfits.empty.mine.open.title"), description: t("outfits.empty.mine.open.description"), ctaLabel: t("outfits.empty.mine.open.cta") }
      : mineStatus === "accepted"
        ? { title: t("outfits.empty.mine.accepted.title"), description: t("outfits.empty.mine.accepted.description") }
        : { title: t("outfits.empty.mine.closed.title"), description: t("outfits.empty.mine.closed.description") };
    ...
    <main className="min-h-screen bg-black px-[var(--app-main-px)] pb-[var(--app-main-pb)] pt-[var(--page-header-top)] text-white">
      <div className="mx-auto max-w-5xl">
        <PageHeader
          ...
          action={<button onClick={() => router.push("/outfits/new")} className="outfit-pressable outfit-primary-action flex shrink-0 items-center justify-center gap-2 rounded-xl bg-orange-500 px-5 py-3 text-sm font-black text-black transition-[background-color,transform] duration-150"><Plus className="h-4 w-4" /> {t("outfits.request")}</button>}
        />

        <div className="mt-[var(--page-header-content-gap)] grid grid-cols-3 gap-1 border-b border-white/[0.08]">
          {tabs.map((tab) => (
            <button key={tab.value} type="button" aria-pressed={scope === tab.value} onClick={() => selectScope(tab.value)} className={`outfit-pressable outfit-tab min-h-11 border-b-2 px-4 py-2 text-sm font-black transition-[border-color,color,transform] duration-150 ${scope === tab.value ? "border-orange-400 text-orange-200" : "border-transparent text-white/45"}`}>{tab.value === "open" ? t("outfits.tab.open") : tab.value === "mine" ? t("outfits.tab.mine") : t("outfits.tab.proposed")}</button>
          ))}
        </div>
        ...
        {loading && !hasCompletedInitialLoad ? (
          ...
          <div className="mt-5 rounded-3xl border border-dashed border-white/15 bg-white/[0.025] px-6 py-20 text-center">
            ...
            {scope === "mine" && mineEmptyState.ctaLabel && (
