...
export function UsernameSetupForm({ initialUsername = "", submitLabel, onSubmit, onSuggestionSelected, onUsernameChange, analyticsSource = "username_settings", showSuggestions = true }: Props) {
  ...
  useEffect(() => {
    ...
    setMessage(tRef.current("username.checking"));
    const timer = window.setTimeout(() => {
      void checkUsernameAvailability(username)
        .then(({ available, reason }) => {
          ...
          setMessage(available ? tRef.current("username.available") : reason || tRef.current("username.unavailable"));
        })
        .catch((error: unknown) => {
          if (!active) return;
          setAvailability("unavailable");
          setMessage(error instanceof Error ? error.message : tRef.current("username.checkError"));
        });
    }, 350);
  ...
  return (
    <div>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="mt-5">
          <p className="text-xs font-black text-gray-400">{t("username.suggestions")}</p>
