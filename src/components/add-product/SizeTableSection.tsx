...
import { canRegisterWithoutSizeTable } from "../../hooks/product-form/helpers";
...
export function SizeTableSection({ form }: SizeTableSectionProps) {
  ...
  const isSizeTableRequired = !canRegisterWithoutSizeTable(
    form.inferredProductCategory
  );
  const [isExtraOpen, setIsExtraOpen] = useState(false);
  const [isDragging, setIsDragging] = useState(false);
  const [isImagePreviewOpen, setIsImagePreviewOpen] = useState(false);
  const isComparisonMode = Boolean(
    form.formData.sizeChartImage && form.formData.extractedTable
  );
  ...
  return (
    <div className="space-y-2">
      ...
      {!form.formData.extractedTable &&
      form.formData.sizeChartImage &&
      !form.isAnalyzingTable ? (
        <div className="text-xs text-amber-300">
          {t("sizeTable.unverified")}
        </div>
      ) : null}
        ...
        <div className={isComparisonMode ? "space-y-3" : ""}>
          {isComparisonMode ? (
            <div className="rounded-xl border border-white/10 overflow-hidden bg-white/[0.03]">
              ...
              <div className="border-t border-white/10 px-3 py-2 text-xs text-gray-400">
                {t("sizeTable.originalHint")}
              </div>
            </div>
          ) : null}
          <div className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.02]">
            <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.04] px-3 py-2">
              <span className="text-xs text-gray-500">
                {form.tableEditingCell
                  ? t("sizeTable.editingHint")
                  : t("sizeTable.editHint")}
              </span>
              ...
              <table className="w-full text-xs text-left">
                {form.formData.extractedTable.headers.length > 0 ? (
                  <thead className="border-b border-white/10">
                    <tr>
                      {form.formData.extractedTable.headers.map(
                        (header, colIdx) => {
                          const isEditing =
                            form.tableEditingCell?.kind === "header" &&
                            form.tableEditingCell.colIdx === colIdx;
                          return (
                            <th
                              key={colIdx}
                              onClick={() =>
                                form.setTableEditingCell({
                                  kind: "header",
                                  colIdx,
                                })
                              }
                            ...
                            >
                              {isEditing ? (
                                <input
                                  autoFocus
                                  defaultValue={header}
                                  aria-label={t("sizeTable.editCell", {
                                    value: header,
                                  })}
                                  onFocus={(event) =>
                                  ...
                                  onBlur={(e) =>
                                    commitTableCell(form, e.target.value)
                                  }
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter")
                                      commitTableCell(
                ...
                <tbody>
                  {form.formData.extractedTable.rows.map((row, rowIdx) => (
                    <tr key={rowIdx} className="border-b border-white/[0.06]">
                      {row.map((cell, colIdx) => {
                        const isEditing =
                          form.tableEditingCell?.kind === "row" &&
                          form.tableEditingCell.rowIdx === rowIdx &&
                          form.tableEditingCell.colIdx === colIdx;
                        return (
                          <td
                            key={colIdx}
      ...
      typeof document !== "undefined"
        ...
        : null}
      {form.formData.extractedTable?.extra?.headers?.length &&
      !form.isAnalyzingTable ? (
        <div className="rounded-xl border border-white/10 overflow-hidden">
          ...
          >
            ...
            <ChevronDown
              className={`h-3.5 w-3.5 transition-transform ${isExtraOpen ? "rotate-180" : ""}`}
            />
          </button>
          {isExtraOpen ? (
            <div className="overflow-x-auto border-t border-white/10">
              <table className="w-full text-xs text-left">
                <thead className="border-b border-white/10">
                  <tr>
                    {form.formData.extractedTable.extra.headers.map(
                      (header, colIdx) => (
                        <th
                          key={colIdx}
                ...
                <tbody>
                  {form.formData.extractedTable.extra.rows.map(
                    (row, rowIdx) => (
                      <tr key={rowIdx} className="border-b border-white/[0.06]">
                        {row.map((cell, colIdx) => (
                          <td
