...
export const isOptionalMetadataCategory = (category: string): boolean =>
  category === "Shoes" ||
  category === "Bag" ||
  category === "JewelryWatch" ||
  category === "FashionAccessory" ||
  category === "Acc" ||
  category === "단종된 상품(빈티지)";
...
export const toPublicUrl = (
  ...
  const url = new URL(
    `${SUPABASE_URL}/storage/v1/${base}/${STORAGE_BUCKET}/${encodedPath}`
  );
...
export const normalizeProduct = (row: ProductRow): Product | null => {
  ...
  return {
    ...
    collectionAddedAt: row.collection_added_at
      ? String(row.collection_added_at)
      : null,
    ...
    styleAttributes: (row.style_attributes ??
      null) as Product["styleAttributes"],
    styleAxes: (row.style_axes ?? null) as Product["styleAxes"],
    styleTagsEvidence: (row.style_tags_evidence ??
      null) as Product["styleTagsEvidence"],
    styleTagsConfidence:
    ...
    styleAxisAnalysisError: row.style_axis_analysis_error
      ...
      : null,
    styleAxisAnalyzedAt: row.style_axis_analyzed_at
      ? String(row.style_axis_analyzed_at)
      : null,
    ...
    humanStyleAttributes: (row.human_style_attributes ??
      null) as Product["humanStyleAttributes"],
    humanStyleAxes: (row.human_style_axes ?? null) as Product["humanStyleAxes"],
    humanStyleTagsEvidence: (row.human_style_tags_evidence ??
      null) as Product["humanStyleTagsEvidence"],
    tagReviewStatus: row.tag_review_status
      ? (String(row.tag_review_status) as Product["tagReviewStatus"])
      : null,
    tagReviewNote: row.tag_review_note ? String(row.tag_review_note) : null,
    reviewedBy: row.reviewed_by ? String(row.reviewed_by) : null,
    reviewedAt: row.reviewed_at || null,
    imageEmbedding:
      typeof row.image_embedding === "string" ||
      Array.isArray(row.image_embedding)
        ? row.image_embedding
        : null,
    targetGender: row.target_gender
      ? (String(row.target_gender) as Product["targetGender"])
      : null,
    humanTargetGender: row.human_target_gender
      ? (String(row.human_target_gender) as Product["humanTargetGender"])
      : null,
