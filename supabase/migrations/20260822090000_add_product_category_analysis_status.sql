alter table public.products
  ...
  add column if not exists category_analysis_status text not null default 'completed';

update public.products
set category_analysis_status = 'completed'
where category_analysis_status is null;
...
alter table public.products add constraint products_category_sub_category_check check (
  ...
  (category in ('Top','Outer','Bottom','DressSkirt','Shoes','Bag','JewelryWatch','FashionAccessory') and (
    ...
    (category = 'Outer' and sub_category in ('후드 집업','블루종·MA-1','레더·라이더스 재킷','수트·블레이저 재킷','가디건','코트·트렌치','경량 패딩·패딩 베스트','사파리·헌팅 재킷','트러커 재킷','스타디움 재킷','나일론·코치 재킷','트레이닝 재킷','아노락 재킷','플리스·뽀글이','기타 아우터')) or
    ...
    (category = 'Shoes' and sub_category in ('스니커즈','스포츠화','구두','부츠·워커','샌들·슬리퍼','패딩·퍼 신발')) or
...
alter table public.products drop constraint if exists products_category_analysis_status_check;
alter table public.products add constraint products_category_analysis_status_check
  check (category_analysis_status in ('pending', 'completed', 'failed'));

comment on column public.products.category_analysis_status is
  'Status of final-storage-image Gemini category analysis.';
