-- The fixed eight-axis centres are computed in application code. Persist the
-- exact vector used at swipe time so historical preference events stay stable
-- when a product is later re-reviewed.
create table if not exists public.user_taste_swipe_events (
...
alter table public.user_taste_swipe_events
  add column if not exists style_profile_snapshot jsonb,
  add column if not exists style_profile_version text;

create index if not exists user_taste_swipe_events_user_created_idx
...
-- Collection RPCs return the complete inputs required by the shared profile
-- calculator. Legacy tag columns remain in products for rollback only and are
...
returns table (
  ...
  human_target_gender text, style_attributes jsonb, style_axes jsonb,
  human_style_attributes jsonb, human_style_axes jsonb,
  style_axes_reviewed_at timestamptz, registered_by text, added_at timestamptz,
  discovered_save_count bigint, size_decision_label text,
  size_decision_row_index integer, size_decision_snapshot jsonb,
  size_decision_sources text[], size_decision_fit text, size_decision_note text,
...
as $$
  select p.id, p.brand, p.name, p.category, p.sub_category, p.url,
    ...
    item.size_decision_fit, item.size_decision_note, item.size_decision_updated_at
