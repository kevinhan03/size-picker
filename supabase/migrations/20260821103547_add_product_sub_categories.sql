...
-- Acc is retired as a parent category. Existing rows get a conservative parent
-- draft; a leaf is intentionally left NULL for administrator review.
update public.products
set category = case
  when name ~* '(백팩|토트.?백|숄더.?백|크로스.?백|메신저|에코.?백|보스턴|더플|웨이스트|힙색|파우치|브리프.?케이스|캐리어|지갑|머니.?클립|클러치)' then 'Bag'
  when name ~* '(시계|팔찌|반지|목걸이|귀걸이|주얼리|jewelry|watch|bracelet|necklace|earring)' then 'JewelryWatch'
  else 'FashionAccessory'
end,
sub_category = null
where category = 'Acc';
