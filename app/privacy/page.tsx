    ...
    >
      ...
      <LegalSection id="retention" title="4. 보유·이용 기간">
        <LegalTable
          ...
          rows={[
            ...
            ["PostHog 제품 분석 이벤트 및 세션 리플레이", "서비스 개선 목적 달성 또는 이용자의 삭제 요청 시까지. 삭제는 공급자의 비동기 처리 절차에 따라 완료됩니다."],
      ...
      <LegalSection id="processors" title="6. 개인정보 처리업무의 위탁">
        <LegalTable
          ...
          rows={[
            ...
            [
              "PostHog, Inc.",
              "페이지 조회와 기능 이용에 대한 제품 분석 및 세션 리플레이",
              "브라우저 익명 식별자 또는 서버에서 해시한 회원 분석 식별자, 페이지·기능 이벤트, 브라우저·기기·IP 및 대략적 지역 정보. 세션 리플레이에서는 모든 입력값을 브라우저에서 마스킹하며 관리자 화면은 기록하지 않습니다. 이메일·이용자명·입력 내용은 제품 분석에 전송하지 않으며 미국 클라우드에서 처리됩니다.",
            ],
            [
              "Google LLC",
      ...
      <LegalSection id="cookies" title="8. 쿠키, 로컬 스토리지 및 행태정보">
        <p>
          ...
          PostHog는 페이지 조회와 클릭·기능 이용 이벤트 및 세션 리플레이를 수집하고 익명 식별자를 브라우저에 저장할 수 있습니다. 세션 리플레이의 모든 입력값은
          브라우저에서 마스킹되며 관리자 화면은 기록하지 않습니다. 로그인하면 서버가 회원 계정 식별자를 복원할 수 없도록 해시한 분석 식별자로 익명 행동을 연결할 수 있습니다. Vercel Web
